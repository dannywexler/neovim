import({ 'leap', 'nest', 'telescope' }, function(p)
    ...
    -- local telescope = p.telescope.builtin
    ...
    --     local text = 'on row: ' .. row
    --     print(text)
    --     local line = unpack(vim.api.nvim_buf_get_lines(0, row, row + 1, true))
    -- print(unpack(line))
    -- print(vim.inspect(line))
    -- text = text .. ' text is: ' .. line
    -- call string.find to check if item is in string
    -- call string.gsub to make the substitution
    -- print(text)
    -- end)
    ...
    -- vim.keymap.set('n', '<leader>j', ':put<CR>', { expr = true, callback = function()
    -- vim.keymap.set('n', '<leader>j', 'p', { callback = function()
    --     print('format after print')
    --     lsp.format()
    -- end })

    -- vim.keymap.set('n', '<leader>k', ':put!<CR>', { callback = function()
    --     lsp.format()
    -- end })

    -- vim.keymap.set('n', '<leader>l', 'V:put<CR>', { callback = function()
    --     lsp.format()
    -- end })
    ...
    p.nest.applyKeymaps {
        { mode = 'n', {
            ...
            { 'j', function() p.leap.leap {} end },
            { 'J', '20<Down>' },
            { 'k', function() p.leap.leap { backward = true } end },
            ...
            { 's', {
                ...
                -- { 's', '<cmd>HopChar2MW<CR>' },
                { 's', function()
                    p.leap.leap { target_windows = require('leap.util').get_enterable_windows() }
                end }
            } },
            { 'U', '<C-r>' },
            -- { 'w', ':HopWordAC<CR>' },
            ...
            { '<A-', {
                ...
                -- { 'e>', '<C-w>h' },
                -- { 'D>', '<cmd>BufferLineMoveNext<CR>' },
                -- { 'E>', '<cmd>WinShift left<CR>' },
                -- { 'f>', '<cmd>BufferLinePick<CR>' },
                { 'g>', ':DiffviewOpen<CR>' },
                { 'i>', '2<C-w><' },
                ...
                { 's>', '<C-w>j' },
                -- { 's>', '<cmd>BufferLineCyclePrev<CR>' },
                -- { 'S>', '<cmd>BufferLineMovePrev<CR>' },
