...
---@field config? boolean | function executed when the plugin loads
...
---@field opts? table | fun(plugin: table, opts: table):table? Opts should be a table (will be merged with parent specs), return a table (replaces parent specs) or should change a table.
...
function PLUG(pluginURL, spec)
    ...
    if (type(config) ~= "function" and config ~= false and opts == nil) then
        -- LOG(pluginURL, "did not have config or opts setup")
        config = true
    end
